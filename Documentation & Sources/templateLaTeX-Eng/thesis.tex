...
\pagestyle{headings}
	Computer science and programming is taught in schools around Romania for at least 30 years, especially in high schools, but also in secondary schools, starting with the \nth{5} grade. Before introducing directly to a programming language, many teachers use a pseudocode language which serves as a mean of understanding programming concepts in a more universal manner, bringing it closer to the natural spoken language.
	
	The purpose of this project is to create an editor and interpreter of pseudocode language created based on concepts similar to C and Python. This application will serve as an educational tool for 
...
Stretches over about 10\% of the paper.
	...
	For a technical point of view, the pseudocode will be created like any other programming languages, having similar elements to existing ones that are used nowadays, but also with specific structural elements which the teachers and students use, bringing it closer to the natural language. Each programming language has a parser and compiler with is comprised of a lexical and a syntactic analyzer which interprets the text based on a set of rules and returns the appropriate result.
